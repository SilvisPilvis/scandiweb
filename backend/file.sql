...
-- Temporarily disable foreign key checks to allow dropping tables out of strict dependency order
...
-- 1. Create tables with no foreign key dependencies, or whose dependencies are
--    already created.
...
-- 3. Create tables that depend on `products` and other already created tables.
...
-- Data insertion - the order of these usually doesn't matter for inserts,
-- but ensure parent tables (e.g., `products`, `categories`) have data before
